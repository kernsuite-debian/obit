...
/* internal prototypes */
/* Get inputs */
ObitInfoList* ImagerIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void ImagerOut (ObitInfoList* outList, ObitErr *err);
...
/* Write history */
void ImagerHistory (gchar *Source, gchar Stok, ObitInfoList* myInput, 
		    ObitUV* inData, ObitImage* outImage, ObitUV* outData, 
		    ObitErr* err);

/* Image statistics */
void ImagerStats (ObitInfoList* myInput, ObitImage *outImage[4], 
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: Imager -input file -output ofile [args]\n");
    fprintf(stderr, "Imager Obit task to image/CLEAN data\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def Imager.in\n");
    fprintf(stderr, "  -output output result file, def Imager.out\n");
...
/*     out2Class Str [6]    output AIPS uv class  [Imager]                */
 		     ...
 		     ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    if (ObitInfoListGetP(myInput, "out2Class", &type, dim, (gpointer)&strTemp)) {
      ...
      strncpy (Aclass, "Imager", 7);
    }
    /* Default for blank */
    if (!strncmp (Aclass, "    ", 4)) strncpy (Aclass, "Imager", 7);
    Aclass[6] = 0;
		 ...
		 ObitErr* err)
{
  ...
  if (chAvg<=0) {chAvg = EChan-BChan+1; chInc = chAvg;}
  /* Update inputs */
  dim[0] = dim[1] = dim[2] = 1;
  ...
  for (ichan = RChan; ichan<=EChan; ichan+=chInc) {
	...
	setOutputData (Source, kstok, myInput, outData, &outImage[istok-bstok], err);
	if (err->error) Obit_traceback_msg (err, routine, inData->name);
	...
	ObitImageFullInstantiate (outImage[istok-bstok], TRUE, err);
	if (err->error) Obit_traceback_msg (err, routine, outImage[istok-bstok]->name);
      }
	...
	ObitDataCopyTable ((ObitData*)outField, (ObitData*)outImage[istok-bstok],
      ...
      /* Fooey */
      ObitImageOpen (outField, OBIT_IO_ReadOnly, err);
      ObitImageClose (outField, err);
      if (err->error) Obit_traceback_msg (err, routine, outField->name);
      /* Insert into output cube */
      ObitImageUtilInsertPlane (outField, outImage[istok-bstok], plane, err);
      if (err->error) Obit_traceback_msg (err, routine, myClean->name);
  ...
  for (istok=bstok; istok<=estok; istok++) {
    ...
    if (istok==bstok)
      ImagerHistory (Source, chStokes[istok], myInput, inData, outImage[istok-bstok], 
		     outData, err);
    else
      ImagerHistory (Source, chStokes[istok], myInput, inData, outImage[istok-bstok], 
		     NULL, err);
...
/*      err    Obit Error stack                                           */
/*----------------------------------------------------------------------- */
void ImagerHistory (gchar *Source, gchar Stoke, ObitInfoList* myInput, 
		    ObitUV* inData, ObitImage* outImage, ObitUV* outData, 
		    ObitErr* err)
{
