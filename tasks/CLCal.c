...
/* internal prototypes */
/* Get inputs */
ObitInfoList* CLCalIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void CLCalOut (ObitInfoList* outList, ObitErr *err);
...
/* Write history */
void CLCalHistory (ObitInfoList* myInput, ObitUV* inData, ObitErr* err);

/* Program globals */
gchar *pgmName = "CLCal";       /* Program name */
gchar *infile  = "CLCal.in" ;   /* File with program inputs */
gchar *outfile = "CLCal.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
  ...
  gchar        *dataParms[] = {  /* Parameters to calibrate/select data */
    ...
    "interMode", "interParm", "doBlank", "allPass", "maxInter", "interNPoly", 
    "refAnt", 
    NULL};

  /* Startup - parse command line */
  err = newObitErr();
  myInput = CLCalIn (argc, argv, err);
  ...
  /* Apply calibration */
  CLTable =  ObitUVSoln2Cal (inData, inData, err);
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;

  /* Write history */
  CLCalHistory (myInput, inData, err); 
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: CLCal -input file -output ofile [args]\n");
    fprintf(stderr, "Obit task to apply an SN table to a CL table \n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def CLCal.in\n");
    fprintf(stderr, "  -output uv data onto which to attach FG table, def CLCal.out\n");
...
void digestInputs(ObitInfoList *myInput, ObitErr *err)
{
  ...
  doCLCal = -1;
  ObitInfoListGetTest(myInput, "doCLCal",  &type, dim, &doCLCal);
  doCalSelect = doCalSelect || (doCLCal>0);
...
ObitUV* getInputData (ObitInfoList *myInput, ObitErr *err)
{
  ...
  gchar        *dataParms[] = {  /* Parameters to calibrate/select data */
    ...
    "doCalSelect", "doCLCal", "gainUse", "doBand", "BPVer", "flagVer", "doPol",
    "Antennas",
     NULL};
